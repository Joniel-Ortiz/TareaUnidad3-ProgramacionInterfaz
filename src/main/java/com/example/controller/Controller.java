...
public class Controller implements ActionListener, MouseListener {
    ...
    public Controller(MenuAgregar menuAgregar, Persona persona, MenuPrincipal menuPrincipal,
        ...
        // Llenar el ComboBox de idiomas desde el Controller para que JFormDesigner no lo borre
        menuConfig.cmbIdiomas.setModel(new DefaultComboBoxModel<>(new String[]{
            "Español", 
            "English", 
            "Français"
        }));
    ...
    public void buscarContactoEnSegundoPlano(String textoBusqueda) {
        ...
        SwingWorker<List<Persona>, Void> worker = new SwingWorker<>() {
            ...
            protected void done() {
                try {
                    List<Persona> resultados = (List<Persona>) get();
                    SwingUtilities.invokeLater(() -> {
                        listModel.clear();
                        listModelFav.clear();
    ...
    public void datosContacto(Object objeto) {
        if (objeto instanceof Persona) {
            ...
            if (persona.isFavorito()) {
                menuDatos.lblNombre.setIcon(cargarIconoSeguro("/com/example/icons/estrella.png"));
            } else {
            ...
            switch (persona.getCategoria()) {
                case "Amigo":
                    menuDatos.categoriaContacto.setIcon(cargarIconoSeguro("/com/example/icons/11325049(1).png"));
                    break;
                case "Familia":
                    menuDatos.categoriaContacto.setIcon(cargarIconoSeguro("/com/example/icons/familia(3)(1).png"));
                    break;
                case "Trabajo":
                    menuDatos.categoriaContacto.setIcon(cargarIconoSeguro("/com/example/icons/760621(1).png"));
                    break;
                default:
    ...
    public void cambiarIdioma(String codigo) {
        try {
            ...
            System.err.println("Ocurrió un error al aplicar el idioma: " + ex.getMessage());
            ex.printStackTrace(); // Esto nos imprimirá el error completo en la consola
        }
    }
    ...
    public void convertirAJson() {
        ...
        try (FileWriter writer = new FileWriter("contactos.json")){
            gson.toJson(todasLasPersonas,writer);
            mostrarNotificacion("Archivo JSON creado exitosamente.");
        } catch (IOException e) {
    ...
    public void cargarJson () {
        ...
        try (FileReader reader = new FileReader("contactos.json")) {
            ...
            todasLasPersonas = gson.fromJson(reader, tipoLista);
            for (Persona persona : todasLasPersonas) {
                listModel.addElement(persona);
                if (persona.isFavorito()) {
                    listModelFav.addElement(persona);
                }
            }

            mostrarNotificacion("Archivo JSON cargado exitosamente.");
        } catch (IOException e) {
            mostrarNotificacion("Error al cargar los datos.");
        }
    }
    ...
    private ImageIcon cargarIconoSeguro(String ruta) {
    java.net.URL url = getClass().getResource(ruta);
    if (url != null) {
        return new ImageIcon(url);
    } else {
        System.err.println("Error: No se pudo encontrar el icono en: " + ruta);
        return null; 
    }
}
